...
load = np.load('./CIOC_Cartpole_trial_1.npy', allow_pickle=True).item()
gamma = load['gamma']
...
data = np.load('../../Demos/Cartpole_Demo.npy', allow_pickle=True).item()
dt = data['dt']
...
# env.initCost(wx=data['wx'], wq=data['wq'], wdx=data['wdx'], wdq=data['wdq'], wu=0.1)
env.initCost(wu=0.1)
...
ax1.set_position([0.215, 0.13, 0.75, 0.78])
plt.show()
