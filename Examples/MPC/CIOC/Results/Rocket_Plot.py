...
load = np.load('./CIOC_Rocket_trial_1.npy', allow_pickle=True).item()
gamma = load['gamma']
...
data = np.load('../../Demos/Rocket_Demo.npy', allow_pickle=True).item()
dt = data['dt']
...
true_parameter = [data['Jx'], data['Jy'], data['Jz'], data['mass'], data['l'], data['max_f_sq'], data['max_tilt_angle'],
                  data['wr'], data['wv'], data['wtilt'], data['ww'], data['wsidethrust']],
env.initConstraints()
...
line_barrierOC, = ax1.plot(loss_trace_barrierOC, color='tab:brown', linewidth=5,)
...
ax1.set_position([0.19, 0.13, 0.78, 0.78])
plt.show()